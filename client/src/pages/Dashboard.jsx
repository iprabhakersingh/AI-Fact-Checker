    ...
    <div>
      <div className='max-w-7xl mx-auto px-4 py-8'>
        ...
        <div className='flex gap-4 '>
          ...
          <button
            onClick={() => setShowUploadATS(true)}
            className="w-full bg-white sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 text-slate-600 border border-dashed border-slate-300 group hover:border-indigo-500 hover:shadow-lg transition-all duration-300 cursor-pointer"
          >
            <UploadCloudIcon className="size-11 p-2.5 bg-gradient-to-br from-indigo-300 to-indigo-500 text-white rounded-full" />
            ...
            return (
              <button key={index} onClick={() => navigate(`/app/builder/${resume._id}`)} className='relative w-full sm:max-w-36 h-48 flex flex-col items-center justify-center rounded-lg gap-2 border group hover:shadow-lg transition-all duration-300 cursor-pointer' style={{ background: `linear-gradient(135deg, ${baseColor}10, ${baseColor}40)`, borderColor: baseColor + '40' }}>
                ...
                <div
                  ...
                  className="absolute bottom-8 right-2 hidden group-hover:flex"
                >
                  <button
                    onClick={() => navigate(`/app/ats/${resume._id}`)}
                    className="text-[11px] px-2 py-1 rounded bg-black/70 text-white hover:bg-black transition"
                  >
                    ATS
                  </button>
                </div>
        ...
        {showUploadATS && (
          <form
            onSubmit={uploadResumeForATS}
            onClick={() => setShowUploadATS(false)}
          ...
          >
            ...
            >
              <h2 className='text-xl font-bold mb-4'>Upload Resume for ATS</h2>
              ...
              <input
                ...
                className='w-full px-4 py-2 mb-4 focus:border-green-600 ring-indigo-600'
                required
              />

              <div>
                <label htmlFor="resume-input-ats" className="block text-sm text-slate-700">
                  Select resume file
                  <div className='flex flex-col items-center justify-center gap-2 border group text-slate-400 border-slate-400 border-dashed rounded-md p-4 py-10 my-4 hover:border-green-500 hover:text-green-700 cursor-pointer transition-colors'>
                    {resume ? (
                      <p className='text-green-700'>{resume.name}</p>
              ...
              <button
                disabled={isLoading}
                className='w-full py-2 bg-green-600 text-white rounded hover:bg-indigo-700 transition-colors flex items-center justify-center gap-2'
              >
                {isLoading && <LoaderCircleIcon className='animate-spin size-4 text-white' />}
                {isLoading ? 'Uploading...' : 'Upload & Analyze ATS'}
              </button>

              <XIcon
                className='absolute top-4 right-4 text-slate-400 hover:text-slate-600 cursor-pointer transition-colors'
                onClick={() => { setShowUploadATS(false); setTitle('') }}
              />
            </div>
          </form>
        )}
